...
options:
  ...
  acl_operation:
    ...
    choices: [all, alter, alter_configs, cluster_actions, create, delete, describe, describe_configs,
...
class ACLResource(object):
    ...
    def __repr__(self):
        return "ACLResource(resource_type: %s, operation: %s, permission_type: %s, name: %s, principal: %s, host: %s)" \
               % (self.resource_type, self.operation, self.permission_type, self.name, self.principal, self.host)
...
def main():
    ...
    module = AnsibleModule(
        argument_spec=dict(
            ...
            acl_resource_type=dict(choices=['topic', 'broker', 'delegation_token', 'group', 'transactional_id'],
                                   default='topic'),

            acl_principal=dict(type='str', required=False),

            acl_operation=dict(
                choices=['all', 'alter', 'alter_configs', 'cluster_actions', 'create', 'delete', 'describe',
                         'describe_configs', 'idempotent_write', 'read', 'write'], required=False),
            ...
            zookeeper_sleep_time=dict(
                type='int', 
                required=False, 
                default=5
            ),

            zookeeper_max_retries=dict(
                type='int', 
                required=False, 
                default=5
            ),
