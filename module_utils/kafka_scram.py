...
ScramMechanism = namedtuple('ScramMechanism', [
    'name', 'int_representation', 'hashname', 'default_iterations', 'min_iterations'])
...
def get_mechanism_from_int(int_representation):
    ...
    if int_representation not in _int_mechanism_map:
        raise ValueError('%s is not a valid mechanism int' % int_representation)
...
def create_salted_password(mechanism_name, password, salt, iterations=None):
    ...
    if iterations < mechanism.min_iterations:
        raise ValueError('Invalid iterations: %d  - Min required %d ' % (iterations, mechanism.min_iterations))

    password = password.encode('utf-8')

    salted_password = hashlib.pbkdf2_hmac(mechanism.hashname, password, salt, iterations)
