...
kafka_zk_tls_hosts = dict()
for host in testinfra.get_hosts(
    ['kafka1_zk_tls'],
    connection='ansible',
    ansible_inventory=os.environ['MOLECULE_INVENTORY_FILE']
):
    kafka_zk_tls_hosts[host] = host.ansible.get_variables()

zookeeper_tls_hosts = dict()
for host in testinfra.get_hosts(
    ['zookeeper-tls'],
    connection='ansible',
    ansible_inventory=os.environ['MOLECULE_INVENTORY_FILE']
):
    zookeeper_tls_hosts[host] = host.ansible.get_variables()
...
def test_quotas_create_sasl_ssl_zk_ssl(host):
    """
    Check if can create quotas
    """
    # Given
    ...
    test_quotas_configuration.update(sasl_ssl_default_configuration)
    test_quotas_configuration.update(zk_ssl_default_configuration)
    ensure_kafka_quotas(
        host,
        test_quotas_configuration
    )
    ...
    time.sleep(0.3)
    # Then
    for host, host_vars in kafka_zk_tls_hosts.items():
        kfk_addr = "%s:9094" % \
            host_vars['ansible_eth0']['ipv4']['address']['__ansible_unsafe']
        check_configured_quotas_kafka(host,
                                      test_quotas_configuration,
                                      kfk_addr)
    for host, host_vars in zookeeper_tls_hosts.items():
        zk_addr = "%s:2181" % \
            host_vars['ansible_eth0']['ipv4']['address']['__ansible_unsafe']
        check_configured_quotas_zookeeper(host,
                                          test_quotas_configuration,
